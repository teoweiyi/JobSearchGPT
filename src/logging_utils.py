...
def setup_logging(logging_config_path="./conf/base/logging.yaml", default_level=logging.INFO):
    ...
    try:
        ...
        logging.basicConfig(
            format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", level=default_level
        )
        logger.info(error)
